% Xóa option '3p' (không hợp lệ với class report) và scrextend
% (xung đột với titlesec/report). Font 13pt được xử lý bởi geometry.
...
\usepackage{xurl}
\usepackage{longtable}  % dùng cho bảng danh mục viết tắt

% ── TikZ / PGFPlots (dùng cho biểu đồ và sơ đồ trong Chương 3) ──────────────
\usepackage{listings}   % Môi trường lstlisting (code highlighting)
\usepackage{tikz}
\usepackage{pgfplots}
...
% \renewcommand{\bibname}{...}  % vô tác dụng với biblatex — biblatex
% dùng \printbibliography[title=...]. Tiêu đề đã được set tại \printbibliography.
...
\input{lstlisting}  % Cấu hình listings: Python, Bash, màu sắc
...
% Tắt cleardoublepage trong phần front matter để tránh trang trắng thừa (twoside)
...
\subfile{Chuong/3a_Phan_tich_thiet_ke}
\subfile{Chuong/3b_Ket_qua_danh_gia}
...
\titlecontents{chapter}
    ...
    {}         % unnumbered format
    {\titlerule*[0.3pc]{.}\contentspage}         % filler-page-format, e.g dots
\chapter{MÃ NGUỒN HỆ THỐNG CINEREC}
